 ...
 */
const EXCLUDED_WALLET_IDS = ["bybit", "com.bybit"];
...
const shouldExcludeProvider = (provider: any): boolean => {
  ...
  try {
    ...
    return EXCLUDED_WALLET_IDS.some(
      (id) => name.includes(id) || rdns.includes(id),
    );
  } catch {
    return false;
  }
...
export const wagmiConfig = createConfig({
  ...
  connectors: [
    ...
    // WalletConnect - enables mobile wallet connections (Trust, Rainbow, etc.)
    walletConnect({
      projectId: process.env.NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID || "",
      metadata: {
        name: "ElementPay",
        description: "Crypto Micro Payments Made Simple",
        url: "https://elementpay.net",
        icons: ["https://elementpay.net/logo.png"],
      },
      showQrModal: true,
    }),
    injected({
