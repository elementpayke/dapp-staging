...
const TransactionRow: FC<TransactionRowProps> = ({ tx }: { tx: ExtendedTx }) => {
  ...
  return (
    <div className="group grid grid-cols-12 gap-4 items-center px-6 py-4 hover:bg-gray-50 transition-colors text-sm border-b border-gray-100">
      ...
      <div className="col-span-2 text-center flex flex-col items-center gap-1">
        ...
        {tx.paymentMethod === 'M-Pesa' && tx.receiptNumber && (
          <span className="text-xs text-gray-500 font-mono">Ref: {tx.receiptNumber}</span>
        )}
      </div>
      {/* Status */}
